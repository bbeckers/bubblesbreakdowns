...
# all acronyms of variables are given in parenthesis
variable_index=regexec(paste('\\(','([A-Z]+)','\\)',sep=''),starthtml)

# extracting all non-zero elements
variable_vector=regmatches(starthtml,variable_index)#[[1]][2]
rm(starthtml,variable_index)

nonzeros=sapply(variable_vector,function(x) length(x)!=0)
variables=variable_vector[nonzeros]
rm(nonzeros)

# names in parenthesis dropped
variables=sapply(variables,function(x) x[2])
...
#         dir.create(dir.var)
        ...
        # write.csv(varhtml,'varthtml.txt') # for inspection of html-text
