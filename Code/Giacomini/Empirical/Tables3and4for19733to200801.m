...
TableMSE_all=[]; TableDMall=[]; TableCWall=[]; xvec=[]; 
for model=[1,4,3,2];  %1=Taylor I, 2=monetary, 3=PPP, 4=UIP
    ...
    for ncountry=1:NC; 
        ...
        if model==4; p1=[i_diff(:,ncountry)]; xvec=[xvec,p1]; end; 
...
disp(' DM statistic roll: MSE rw minus MSE model'); 
disp('     TaylorI       UIP       ppp       monetary');
disp([country_name, num2str(-TableMSE_all')]),
