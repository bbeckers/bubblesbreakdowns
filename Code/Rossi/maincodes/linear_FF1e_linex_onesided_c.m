function result=linear_FF1e_linex_onesided(y,x,R,h,types,a); 
...
for j=1:rows(y)-R-(h-1); 
   ...
   f_in_seq     = [f_in_seq;    Linex( olsres(  y(h:R+j-1,1),x(1:R+j-h,:)  ) ,a)];     
   f_oos_seq    = [f_oos_seq  ; Linex(y(R+j+h-1,1)-x(R+j-1,:)  *b_seq' ,a)];    
   f_in_split   = [f_in_split;  Linex( olsres(  y(h:R,1), x(1:R-h+1,:)  ),a)];       
   f_oos_split  = [f_oos_split; Linex(y(R+j+h-1,1)-x(R+j-1,:)  *b_split',a)]; 
   f_in_roll    = [f_in_roll;   Linex( olsres(  y(j+h-1:R+j-1,1),x(j:R+j-h,:)  ),a )]; 
   f_oos_roll   = [f_oos_roll ; Linex(y(R+j+h-1,1)-x(R+j-1,:)  *b_roll', a)]; 
end; 
...
% perform FF test
pai=rows(yf_roll)/R; 
[FFseq  ,pvFFseq  ] = FF_test(f_oos_seq     -f_in_seq   ,( 1    )     );
[FFsplit,pvFFsplit] = FF_test(f_oos_split   -f_in_split , 1+(pai) ); 
if pai>1;
[FFroll ,pvFFroll ] = FF_test(f_oos_roll    -f_in_roll  , 2/(3*pai)   ); 
else; 
[FFroll ,pvFFroll ] = FF_test(f_oos_roll    -f_in_roll  , 1-(1/3)*(pai^2)   ); 
end;

    str=['load cvlinex']; eval(str); 
    str=['cv= cvlinex;']; eval(str); 

%plot(f_oos_roll    -f_in_roll); adsfsdf

if FFseq>cvlinex(1,1);   pvFFseq = 0;   else; pvFFseq=1; end;
if FFsplit>cvlinex(1,2); pvFFsplit = 0; else; pvFFsplit=1; end;
if FFroll>cvlinex(1,3);  pvFFroll= 0;   else; pvFFroll=1; end;
...
if types=='tval'; result=[FFseq, FFsplit, FFroll]; end;
