function result=linear_FF1e_new(y,x,R,h,types); 
...
% perform FF test
pai=rows(yf_roll)/R; 
[FFseq  ,pvFFseq  ] = FF_test_McC3(f_oos_seq,   f_in_seq   ,( 1    )     );
[FFsplit,pvFFsplit] = FF_test_McC3(f_oos_split, f_in_split , 1+(pai) ); 
if pai>1; 
[FFroll ,pvFFroll ] = FF_test_McC3(f_oos_roll,  f_in_roll  , 2/(3*pai)   ); 
else; 
[FFroll ,pvFFroll ] = FF_test_McC3(f_oos_roll,  f_in_roll  , 1-(1/3)*(pai^2)   ); 
end;
...
if types=='forc'; result=[yf_seq, yf_split, yf_roll, true]; end;
