function varargout = myfilter(Inp,Range,Order,varargin)
...
elseif ~isempty(opt.cutoff)
    ...
    if isempty(opt.lambda) ...
            || (ischar(opt.lambda) && strcmpi(opt.lambda,'auto'))
        if freq == 0
            utils.error('tseries', ...
